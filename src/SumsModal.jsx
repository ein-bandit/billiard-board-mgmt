...
export default class SumsModal extends React.Component {
    ...
    constructor(props) {
        ...
        this.props.sums.forEach(function (s, idx) {
            ...
            if (pos > -1) {
                tables[pos].sum += GetPrice(s.timeActive);
                tables[pos].entries.push(GetFormattedPrice(s.timeActive));
            } else {
                tables.push({ nr: s.nr, sum: GetPrice(s.timeActive), entries: [GetFormattedPrice(s.timeActive)] });
            }
            total += GetPrice(s.timeActive);
        });
